...
### Added
- **YouTube quality picker**: popup reads `ytInitialPlayerResponse.streamingData.formats` and displays available muxed qualities (360p / 480p / 720p HD) with file size estimates. User selects a quality and the extension downloads it directly via `chrome.downloads.download` (no "access denied", proper cookie auth). A "Record live (1080p+)" option using MediaRecorder is always available at the bottom of the list.
